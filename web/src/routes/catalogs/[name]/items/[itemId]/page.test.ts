...
import { createMockItem, createMockCatalog, createMockSchema } from '$lib/test-utils/fixtures';
...
describe('Item Detail Page (+page)', () => {
	...
	describe('Data Loading', () => {
		...
		it('loads catalog to get schema name', async () => {
			...
			const mockCatalog = createMockCatalog({ name: 'TestCat', schema_ref: 'TestSchema' });
		...
		it('finds dimension info for attribute', () => {
			const mockSchema = createMockSchema({
				dimensions: [
					...
					{ name: 'size', type: 'enum', required: true, values: ['S', 'M', 'L'] }
				]
			});

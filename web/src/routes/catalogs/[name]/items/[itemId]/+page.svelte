      ...
      <Card>
        <CardHeader>
          <div class="flex items-start justify-between">
            ...
            <Button href="/catalogs/{catalogName}/items/{itemId}/edit"
              >Edit Item</Button
            >
          </div>
        </CardHeader>
      ...
      {#if item.metadata && Object.keys(item.metadata).length > 0}
        <Card>
          <CardHeader>
            ...
            <CardDescription
              >Additional information about this item</CardDescription
            >
          </CardHeader>
          <CardContent>
            <pre
              class="text-xs bg-muted p-3 rounded overflow-auto">{JSON.stringify(
                item.metadata,
                null,
                2,
              )}</pre>

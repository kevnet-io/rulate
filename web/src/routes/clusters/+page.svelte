<script lang="ts">
  ...
  async function loadOptions() {
    try {
      ...
      if (rulesets.length > 0) {
        ...
        selectedClusterRuleset = rulesets[0].name.replace(
          "_rules",
          "_cluster_rules",
        );
      }
  ...
  async function evaluateClusters() {
    ...
    try {
      ...
      error =
        err instanceof Error ? err.message : "Failed to evaluate clusters";
      analysis = null;
    } finally {
      loading = false;
    }
  ...
  function getClusterRelationships(clusterId: string): ClusterRelationship[] {
    if (!analysis) return [];
    return analysis.relationships.filter((r) => r.cluster_id === clusterId);
  }
...
<div class="container mx-auto px-4 py-8">
  <div class="mb-8">
    ...
    <p class="text-muted-foreground">
      Find compatible sets of items in your catalog
    </p>
  </div>
    ...
    <Card>
      ...
      <CardContent>
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <label for="min-size" class="block text-sm font-medium mb-2"
        ...
        <div class="mt-4 flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            <input
              ...
              class="w-4 h-4"
            />
            <label for="max-clusters" class="text-sm"
              >Limit to top 20 clusters</label
            >
          </div>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div class="text-center">
              <div class="text-3xl font-bold text-primary">
                {analysis.avg_cluster_size.toFixed(1)}
              </div>
              <div class="text-sm text-muted-foreground">Avg Size</div>
            </div>
            <div class="text-center">
              <div class="text-3xl font-bold text-primary">
                {analysis.total_items_covered}
              </div>
        ...
        <h2 class="text-2xl font-bold">
          ...
          {#if analysis.clusters.filter((c) => c.is_maximum).length > 0}
            <span class="text-sm text-muted-foreground font-normal">
              ({analysis.clusters.filter((c) => c.is_maximum).length} maximum @ {analysis.max_cluster_size}
              items)
            </span>
          {/if}
        </h2>
            ...
            <CardContent>
              ...
              <div class="mb-4">
                <h4 class="font-semibold text-sm mb-2">
                  Rules: {cluster.rule_evaluations.filter((r) => r.passed)
                    .length}/{cluster.rule_evaluations.length} passed
                </h4>
                <div class="space-y-1">
                ...
                <div>
                  ...
                  <div class="space-y-1">
                    {#each rels.slice(0, 5) as rel}
                      {@const relatedCluster = getRelatedCluster(
                        rel.related_cluster_id,
                      )}
                      <div class="flex items-center gap-2 text-sm">
                        ...
                        {#if relatedCluster}
                          <button
                            type="button"
                            onclick={() =>
                              scrollToCluster(rel.related_cluster_id)}
                        ...
                        {:else}
                          <span class="text-muted-foreground">
                            {rel.related_cluster_id.slice(0, 8)}... ({rel.overlap_size}
                            shared)
                          </span>
                        {/if}
                      </div>

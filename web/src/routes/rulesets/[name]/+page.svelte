...
<div class="container mx-auto px-4 py-8">
  <div class="mb-6">
    <Button href="/rulesets" variant="ghost" size="sm"
      >← Back to RuleSets</Button
    >
  </div>
    ...
    <div class="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription
            >Compatibility conditions for item pairs</CardDescription
          >
        </CardHeader>
        <CardContent>
          {#if ruleset.rules.length === 0}
            <p class="text-muted-foreground">
              No rules defined in this ruleset
            </p>
          {:else}
            <div class="space-y-4">
                ...
                <div class="border rounded-lg p-4">
                  ...
                  {#if rule.type === "exclusion"}
                    <p class="text-xs text-muted-foreground mt-2">
                      Items are <strong>incompatible</strong> if this condition is
                      TRUE
                    </p>
                  {:else}
                    <p class="text-xs text-muted-foreground mt-2">
                      Items are compatible <strong>only if</strong> this condition
                      is TRUE
                    </p>
                  {/if}
                </div>

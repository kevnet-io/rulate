...
<script lang="ts">
	...
	function updateDimension(index: number, updates: Partial<Dimension>) {
		dimensions = dimensions.map((dim, i) => (i === index ? { ...dim, ...updates } : dim));
	}
				...
				<CardHeader>
					<div class="flex items-start justify-between">
						<div>
							...
							<CardDescription>Define the attributes that items can have</CardDescription>
						</div>
						<Button type="button" onclick={addDimension} size="sm">Add Dimension</Button>
					</div>
				</CardHeader>
				<CardContent>
					{#if dimensions.length === 0}
									...
									<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
										...
										<div>
											...
											<input
												...
												bind:value={dim.name}
												onchange={(e) =>
													updateDimension(index, { name: e.currentTarget.value })}
										...
										<div class="flex items-center gap-2">
											<input
												...
												onchange={(e) =>
													updateDimension(index, { required: e.currentTarget.checked })}

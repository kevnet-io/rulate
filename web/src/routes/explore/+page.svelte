...
<script lang="ts">
	...
	async function exploreItem(itemId: string) {
		...
		try {
			...
			const results = await api.evaluateItem(itemId, selectedCatalog, selectedRuleset);
	...
	function getOtherItemId(result: ComparisonResult): string {
		return result.item1_id === currentItem?.item_id ? result.item2_id : result.item1_id;
	}
	...
	<div class="space-y-6">
		<Card>
			<CardHeader>
				...
				<CardDescription>Select catalog, ruleset, and item to explore</CardDescription>
			</CardHeader>
			<CardContent>
				<div class="grid gap-4 md:grid-cols-4">
					...
					<div>
						<label for="item" class="block text-sm font-medium mb-2">Current Item</label>
						<select
							id="item"
					...
					<div class="flex items-end">
						<Button onclick={() => exploreItem(selectedItemId)} disabled={loading} class="w-full">
			...
			{#if compatibleResults.length > 0}
				<Card>
					<CardHeader>
						...
						<CardDescription>Click on any item to explore its compatibility</CardDescription>
					</CardHeader>
					<CardContent>

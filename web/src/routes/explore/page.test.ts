...
describe('Explore Page (+page)', () => {
	...
	describe('Exploration Logic', () => {
		...
		it('passes all three selections to API', async () => {
			...
			const result = await api.api.evaluateItem('item1', 'catalog1', 'ruleset1');

			expect(api.api.evaluateItem).toHaveBeenCalledWith('item1', 'catalog1', 'ruleset1');
		...
		it('finds current item from items list', () => {
			...
			const currentItem = items.find((i) => i.item_id === selectedItemId) || null;
	...
	describe('Other Item Resolution', () => {
		it('extracts other item ID when current is item1', () => {
			...
			const otherId = result.item1_id === currentItemId ? result.item2_id : result.item1_id;
	...
	describe('Page Title and Display', () => {
		...
		it('displays subtitle', () => {
			const subtitle = 'Explore item compatibility interactively - click on any item to see what it\'s compatible with';
	...
	describe('Catalog Change Behavior', () => {
		it('loads items when catalog changes', async () => {
			...
			const items = await api.api.getItems(selectedCatalog);
		...
		it('resets results when catalog changes', () => {
			let currentItem: any = { item_id: 'item1' };
			let compatibleResults: any[] = [{ compatible: true }];

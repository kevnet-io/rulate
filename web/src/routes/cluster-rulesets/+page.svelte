<script lang="ts">
  ...
  async function loadClusterRuleSets() {
    try {
      ...
      error =
        err instanceof Error ? err.message : "Failed to load cluster rulesets";
  ...
  async function deleteClusterRuleSet() {
    ...
    try {
      ...
      toastStore.success(
        `Cluster RuleSet "${clusterRulesetToDelete}" deleted successfully`,
      );
      await loadClusterRuleSets();
    } catch (err) {
      const errorMsg =
        err instanceof Error ? err.message : "Failed to delete cluster ruleset";
  ...
  <div class="flex items-center justify-between mb-8">
    <div>
      ...
      <p class="text-muted-foreground">
        Define cluster-level validation rules for compatible sets
      </p>
    </div>
  </div>
          ...
          <CardHeader>
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                <CardDescription
                  >Version {clusterRuleset.version}</CardDescription
                >
              </div>
              <Badge variant="secondary"
                >{clusterRuleset.rules.length} rules</Badge
              >
            </div>
          </CardHeader>
          <CardContent>
            <div class="space-y-3">
              <div>
                <p class="text-sm text-muted-foreground mb-1">
                  Schema: <span class="font-medium text-foreground"
                    >{clusterRuleset.schema_name}</span
                  >
                </p>
                <p class="text-sm text-muted-foreground mb-2">
                  Pairwise: <span class="font-medium text-foreground"
                    >{clusterRuleset.pairwise_ruleset_name}</span
                  >
                </p>
              </div>
              ...
              <div class="flex gap-2 pt-2">
                <Button
                  href="/cluster-rulesets/{clusterRuleset.name}"
                  variant="outline"
                ...
                <Button
                  variant="destructive"
                  size="sm"
                  onclick={() => confirmDelete(clusterRuleset.name)}
                  >Delete</Button
                >
              </div>
            </div>
...
<ConfirmModal
  ...
  confirmText="Delete"
  isDanger={true}
  details={clusterRulesetToDelete
    ? { Name: clusterRulesetToDelete }
    : undefined}
/>

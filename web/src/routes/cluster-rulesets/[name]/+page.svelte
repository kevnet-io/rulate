<script lang="ts">
  ...
  async function loadClusterRuleSet() {
    try {
      ...
      error =
        err instanceof Error ? err.message : "Failed to load cluster ruleset";
    } finally {
      loading = false;
    }
  }
...
<div class="container mx-auto px-4 py-8">
  <div class="mb-6">
    <Button href="/cluster-rulesets" variant="ghost" size="sm"
      >← Back to Cluster RuleSets</Button
    >
  </div>
    ...
    <div class="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription
            >Validation rules for compatible sets of items</CardDescription
          >
        </CardHeader>
        <CardContent>
          {#if clusterRuleset.rules.length === 0}
            <p class="text-muted-foreground">
              No rules defined in this cluster ruleset
            </p>
          {:else}
            <div class="space-y-4">
              {#each clusterRuleset.rules as rule}
                <div class="border rounded-lg p-4">
                  ...
                  <div class="bg-muted/50 rounded p-3 mt-2">
                    ...
                    <pre
                      class="text-xs font-mono whitespace-pre-wrap">{formatCondition(
                        rule.condition,
                      )}</pre>
                  </div>
                  ...
                  {#if rule.type === "exclusion"}
                    <p class="text-xs text-muted-foreground mt-2">
                      Set is <strong>invalid</strong> as a cluster if this condition
                      is TRUE
                    </p>
                  {:else}
                    <p class="text-xs text-muted-foreground mt-2">
                      Set forms a valid cluster <strong>only if</strong> this condition
                      is TRUE
                    </p>
                  {/if}
                </div>

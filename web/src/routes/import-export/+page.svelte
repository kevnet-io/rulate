<script lang="ts">
  ...
  let importType = $state<
    "all" | "schemas" | "rulesets" | "cluster-rulesets" | "catalogs"
  >("all");
  ...
  async function exportClusterRuleSets() {
    try {
      ...
      error =
        e instanceof Error ? e.message : "Failed to export cluster rulesets";
    } finally {
      loading = false;
    }
  }
    ...
    try {
      ...
      switch (importType) {
        case "all":
          ...
          break;
        case "schemas":
          result = await api.importSchemas(
            Array.isArray(data) ? data : [data],
            skipExisting,
          );
          break;
        case "rulesets":
          result = await api.importRuleSets(
            Array.isArray(data) ? data : [data],
            skipExisting,
          );
          break;
        case "cluster-rulesets":
          result = await api.importClusterRuleSets(
            Array.isArray(data) ? data : [data],
            skipExisting,
          );
          break;
        case "catalogs":
          result = await api.importCatalogs(
            Array.isArray(data) ? data : [data],
            skipExisting,
          );
          break;
...
<div class="container mx-auto px-4 py-8">
  <div class="mb-6">
    ...
    <p class="text-gray-600">
      Export your data to JSON files or import data from previously exported
      files.
    </p>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          Download your data as JSON files. You can export all data at once or
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          Upload a JSON file to import data into Rulate. The file should be in
          the same format as exported files.
        </Card.Description>
      </Card.Header>
  ...
  <div class="mt-8">
    <Card.Root>
      ...
      <Card.Content class="space-y-2 text-sm text-muted-foreground">
        <p>
          <strong class="text-foreground">Export:</strong> Creates JSON files containing
          your data. Use this to back up your data or transfer it between instances.
        </p>
        <p>
          <strong class="text-foreground">Import:</strong> Loads data from JSON files.
          The file format must match the exported format.
        </p>
        <p>
          <strong class="text-foreground">Skip Existing:</strong> When enabled, items
          with names that already exist will be skipped. When disabled, the import
          will fail if duplicates are found.
        </p>
        <p>
          <strong class="text-foreground">Dependencies:</strong> When importing, ensure
          dependencies are imported in order: Schemas → RuleSets → Cluster RuleSets
          → Catalogs. The "All Data" import handles this automatically.
        </p>
      </Card.Content>
    </Card.Root>
  </div>

	...
	describe('Catalog Endpoints', () => {
		...
		describe('updateCatalog', () => {
			it('updates an existing catalog', async () => {
				mockFetch.mockResolvedValueOnce({
					ok: true,
					json: async () => ({ id: 1, name: 'test_catalog', schema_name: 'test_schema' })
				});

				await apiClient.updateCatalog('test_catalog', { description: 'Updated' });
	...
	describe('Item Endpoints', () => {
		...
		describe('updateItem', () => {
			it('updates an existing item', async () => {
				mockFetch.mockResolvedValueOnce({
					ok: true,
					json: async () => ({ id: 1, item_id: 'item-001', name: 'Updated Item' })
				});

				await apiClient.updateItem('test_catalog', 'item-001', { name: 'Updated Item' });
	...
	describe('ClusterRuleSet Endpoints', () => {
		...
		describe('getClusterRuleSet', () => {
			it('fetches a single cluster ruleset by name', async () => {
				...
				const result = await apiClient.getClusterRuleSet('test_cluster_ruleset');
		...
		describe('updateClusterRuleSet', () => {
			it('updates an existing cluster ruleset', async () => {
				mockFetch.mockResolvedValueOnce({
					ok: true,
					json: async () => ({ id: 1, name: 'test_cluster_ruleset', version: '2.0.0' })
				});

				await apiClient.updateClusterRuleSet('test_cluster_ruleset', { version: '2.0.0' });
	...
	describe('Evaluation Endpoints', () => {
		...
		describe('evaluateMatrix', () => {
			it('generates a compatibility matrix', async () => {
				...
				const result = await apiClient.evaluateMatrix('test_catalog', 'test_ruleset');
		...
		describe('evaluateItem', () => {
			it('evaluates an item against all others in catalog', async () => {
				...
				const result = await apiClient.evaluateItem('test_catalog', 'test_ruleset', 'item-001');
	...
	describe('Import/Export Endpoints', () => {
		...
		describe('importAll', () => {
			it('imports all data', async () => {
				const mockImportData = { schemas: [], rulesets: [], catalogs: [], cluster_rulesets: [] };
				const mockResult = { imported: 0 };
		...
		describe('exportAll', () => {
			it('exports all data', async () => {
				const mockExport = { schemas: [], rulesets: [], catalogs: [], cluster_rulesets: [] };

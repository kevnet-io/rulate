...
import { updateAttribute, getListValueAsString, initializeAttributes } from './attributes';
...
describe('Form Attributes Utilities', () => {
	describe('updateAttribute', () => {
		...
		describe('List type', () => {
			it('parses comma-separated strings', () => {
				...
				const attrs = { tags: [] };
				const result = updateAttribute(attrs, 'tags', 'red, blue, green', listDimension);
			...
			it('trims whitespace from list items', () => {
				const listDimension: Dimension = { name: 'tags', type: 'list' };
				const attrs = { tags: [] };
				const result = updateAttribute(attrs, 'tags', '  a  ,  b  ,  c  ', listDimension);
			...
			describe('Integer list items', () => {
				...
				it('converts list items to integers', () => {
					const attrs = { numbers: [] };
					const result = updateAttribute(attrs, 'numbers', '1, 2, 3', intListDimension);
			...
			describe('Float list items', () => {
				...
				it('converts list items to floats', () => {
					const attrs = { values: [] };
					const result = updateAttribute(attrs, 'values', '1.5, 2.5, 3.5', floatListDimension);
			...
			describe('Boolean list items', () => {
				...
				it('converts list items to booleans', () => {
					const attrs = { flags: [] };
					const result = updateAttribute(attrs, 'flags', 'true, false, true', boolListDimension);
				...
				it('treats non-true strings as false', () => {
					const attrs = { flags: [] };
					const result = updateAttribute(attrs, 'flags', 'yes, no, 1', boolListDimension);
		...
		describe('Enum type', () => {
			it('sets enum value directly', () => {
				const enumDimension: Dimension = { name: 'status', type: 'enum', values: ['active', 'inactive'] };
				const attrs = { status: '' };
				const result = updateAttribute(attrs, 'status', 'active', enumDimension);
		...
		describe('String type', () => {
			it('sets string value directly', () => {
				...
				const attrs = { name: '' };
				const result = updateAttribute(attrs, 'name', 'John Doe', stringDimension);
	...
	describe('initializeAttributes', () => {
		...
		it('initializes enum with no values to empty string', () => {
			const dimensions: Dimension[] = [{ name: 'status', type: 'enum', values: [] }];
			const result = initializeAttributes(dimensions);

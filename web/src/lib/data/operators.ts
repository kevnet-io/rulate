...
export const PAIRWISE_OPERATORS: Record<string, OperatorMetadata> = {
  equals: {
    ...
    examples: [
      "Items must have the same color",
      "Items must be from the same season",
    ],
    icon: "=",
  },
  ...
  has_different: {
    ...
    examples: [
      "Items must have different colors",
      "Items must have different layers",
    ],
    icon: "≠",
  },
  ...
  abs_diff: {
    ...
    category: "comparison",
    description:
      "Check if the absolute difference between numeric values meets a condition",
    parameters: [
      {
        name: "field",
        type: "field",
    ...
    examples: [
      "Formality levels differ by more than 2",
      "Prices differ by less than 10",
    ],
    icon: "|Δ|",
  },
  ...
  has_value: {
    ...
    examples: [
      "At least one item is waterproof",
      "At least one item is formal",
    ],
    icon: "∃",
  },
...
export const CLUSTER_OPERATORS: Record<string, OperatorMetadata> = {
  unique_values: {
    ...
    examples: [
      "All items must have different body zones",
      "Each item unique category",
    ],
    icon: "≠*",
  },

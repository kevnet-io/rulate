<script lang="ts">
  ...
  const isWardrobeSchema = $derived(
    schemaName?.toLowerCase().includes("wardrobe"),
  );
  ...
  type InsertionContextType =
    (typeof InsertionContext)[keyof typeof InsertionContext];

  function detectInsertionContext(
    textValue: string,
    cursorPos: number,
  ): InsertionContextType {
    ...
    if (isInsideArray) {
      ...
      const hasLogicalOp = logicalOps.some((op) =>
        beforeCursor.includes(`"${op}"`),
      );
  ...
  function generateOperatorTemplate(operator: OperatorMetadata): string {
    if (operator.parameters.some((p) => p.type === "conditions")) {
      ...
      operator.parameters.length === 1 &&
      operator.parameters[0].type === "field"
    ) {
      // Field-only operator
      return `{\n  "${operator.name}": {\n    "field": ""\n  }\n}`;
    } else {
    ...
    switch (context) {
      ...
      case InsertionContext.INSIDE_ARRAY: {
        ...
        updateCursorPosition(
          start + (needsCommaPrefix ? 2 : 0) + template.length,
        );
        break;
      }
  ...
  const templateCategories = $derived([
    ...
    { value: "cluster", label: "Cluster" },
    ...(isWardrobeSchema
      ? [{ value: "wardrobe", label: "Wardrobe Examples" }]
      : []),
  ]);
  ...
  function insertTemplate(
    ...
    switch (context) {
      ...
      case InsertionContext.INSIDE_ARRAY: {
        ...
        updateCursorPosition(
          start + (needsCommaPrefix ? 2 : 0) + templateJson.length,
        );
        break;
      }
      ...
      default: {
        ...
        updateCursorPosition(
          start + (needsComma ? 2 : 0) + templateJson.length,
        );
        break;
      }
    }
      ...
      <Card class="h-full">
        <CardHeader>
          ...
          <div class="flex gap-2 mb-4 border-b">
            <button
              type="button"
              class="px-4 py-2 text-sm font-medium border-b-2 transition-colors {sidebarTab ===
              'operators'
                ...
                >
                  <div class="flex items-start justify-between mb-2">
                    <div class="flex-1">
                      ...
                      <Badge
                        variant={template.category === "exclusion"
                          ? "destructive"
                          : "default"}
                        class="text-xs mt-1"
                      >

...
test.describe("Explorer Page Workflow", () => {
  test("should navigate to explorer from dashboard", async ({ page }) => {
    ...
    const exploreLink = page
      .locator('a:has-text("Explore"), a:has-text("Explorer")')
      .first();
    const isVisible = await exploreLink
  ...
  test("should display explorer controls", async ({ page }) => {
    ...
    const isVisible = await setupCard
      .isVisible({ timeout: 3000 })
      .catch(() => false);

    // If available, verify controls exist
    if (isVisible) {
      const catalogSelect = page
        .locator('select[id*="catalog"], label:has-text("Catalog")')
        .first();
      const rulesetSelect = page
        .locator('select[id*="ruleset"], label:has-text("RuleSet")')
        .first();

      const hasCatalog = await catalogSelect
        .isVisible({ timeout: 2000 })
        .catch(() => false);
      const hasRuleset = await rulesetSelect
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test("should display loading state during evaluation", async ({ page }) => {
    ...
    const buttonExists = await exploreButton
      .isVisible({ timeout: 2000 })
      .catch(() => false);
  ...
  test("should handle error states gracefully", async ({ page }) => {
    ...
    if (
      ...
      const errorVisible = await page
        .locator("text=select, text=required, text=error")
        .first()
        .isVisible({ timeout: 1000 })
        .catch(() => false);
  ...
  test("should verify navigation links work", async ({ page }) => {
    ...
    if (hasNav) {
      ...
      const homeLink = page
        .locator(
          'a:has-text("Home"), a:has-text("Dashboard"), a:has-text("Rulate")',
        )
        .first();

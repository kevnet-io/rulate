...
test.describe("Toast Notifications", () => {
  test("should show success toast after creating a schema", async ({
    page,
  }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/schemas/${schemaName}`), {
      timeout: 10000,
    });
  });
  ...
  test("should show error toast on API failure", async ({ page }) => {
    ...
    await expect(page.locator("text=/error|already exists/i")).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe("Modal Confirmations", () => {
  ...
  test("should show confirmation modal when deleting schema", async ({
    page,
  }) => {
  ...
  test("should delete item when confirming", async ({ page }) => {
    ...
    const initialCount = await page
      .locator('[data-testid="schema-card"], .space-y-6 > div > div > div')
      .count();
...
test.describe("Empty States", () => {
  test("should show helpful empty state when no schemas exist", async ({
    page,
  }) => {

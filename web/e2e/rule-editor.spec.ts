...
test.describe('RuleEditor Integration', () => {
	...
	test('should display RuleEditor when creating a ruleset', async ({ page }) => {
		// Add a rule
	...
	test('should switch between Operators and Templates tabs', async ({ page }) => {
		// Add a rule
		await page.click('button:has-text("Add Rule")');
		...
		await expect(page.locator('input[placeholder="Search operators..."]')).toBeVisible();
	});
	...
	test('should show wardrobe templates for wardrobe schema', async ({ page }) => {
		...
		const wardrobeOption = options.find(opt => opt.toLowerCase().includes('wardrobe'));
		...
		if (wardrobeOption) {
			...
			const templateOptions = await templateCategorySelect.locator('option').allTextContents();

			// Check if wardrobe option exists
			const hasWardrobe = templateOptions.some(opt => opt.toLowerCase().includes('wardrobe'));
	...
	test('should toggle sidebar visibility', async ({ page }) => {
		...
		await expect(page.locator('input[placeholder="Search operators..."]')).not.toBeVisible();
	...
	test('should create ruleset with rule from template', async ({ page }) => {
		...
		// Submit form
		await page.waitForSelector('button[type="submit"]:has-text("Create RuleSet")', { timeout: 5000 });
		await page.click('button[type="submit"]:has-text("Create RuleSet")');

		// Should show success toast
		await expect(page.locator('[role="alert"]')).toContainText('created successfully');
	...
	test('should filter operators by category', async ({ page }) => {
		...
		const allOperators = await page.locator('button:has-text("Insert")').count();

		// Select Comparison category
		await page.locator('select').filter({ hasText: /All Operators/ }).selectOption('comparison');

		// Should have fewer operators
		const comparisonOperators = await page.locator('button:has-text("Insert")').count();
	...
	test('should search operators by name', async ({ page }) => {
		...
		// Get initial count
		const initialCount = await page.locator('button:has-text("Insert")').count();

		// Search for specific operator
		await page.fill('input[placeholder="Search operators..."]', 'equals');

		// Should have fewer results
		const searchResults = await page.locator('button:has-text("Insert")').count();
...
test.describe('RuleEditor Accessibility', () => {
	test('should have proper ARIA labels', async ({ page }) => {
		...
		await expect(page.locator('button:has-text("Format")')).toHaveAttribute('title');
...
test.describe('Smart Context-Aware Insertion', () => {
	...
	test('should insert into empty array with smart context', async ({ page }) => {
		const textarea = page.locator('textarea').first();
	...
	test('should wrap simple condition when inserting with Insert button', async ({ page }) => {
		const textarea = page.locator('textarea').first();
	...
	test('should replace content with Replace button', async ({ page }) => {
		...
		const firstOperatorCard = page.locator('div:has(> button:has-text("Insert"))').first();

		// Within that card, find the Replace button
		const replaceButton = firstOperatorCard.locator('button:has-text("Replace")').first();
	...
	test('should show error toast for invalid JSON when formatting', async ({ page }) => {
		const textarea = page.locator('textarea').first();
	...
	test('should show info toast when JSON is already formatted', async ({ page }) => {
		const textarea = page.locator('textarea').first();
  ...
  "equals": {
    "field": "color"
		...
		await expect(page.locator('[role="alert"]')).toContainText('already formatted');
	});

	test('should have both Insert and Replace buttons for operators', async ({ page }) => {

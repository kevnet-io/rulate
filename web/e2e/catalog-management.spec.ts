...
test.describe("Catalog Management Workflow", () => {
  ...
  test("should navigate to catalog creation page", async ({ page }) => {
    ...
    const createButton = page
      .locator(
        'button:has-text("Create Catalog"), a:has-text("Create Catalog")',
      )
      .first();
  ...
  test("should view catalog items page", async ({ page }) => {
    ...
    const isVisible = await viewButton
  ...
  test("should display catalog list with proper structure", async ({
    page,
  }) => {
    // Navigate to catalogs
    await page.goto("/catalogs");
  ...
  test("should navigate back from item creation to catalog", async ({
    page,
  }) => {
    ...
    const addItemButton = page
      .locator('a:has-text("Add Item"), a:has-text("new")')
      .first();
    const isVisible = await addItemButton
    ...
    if (isVisible) {
      ...
      const backButton = page
        .locator(
          'button:has-text("Cancel"), a:has-text("Back"), button:has-text("Back")',
        )
        .first();
  ...
  test("should verify catalog cards display required information", async ({
    page,
  }) => {
    // Navigate to catalogs
    await page.goto("/catalogs");
    ...
    const catalogCards = page
      .locator('[class*="card"], [class*="grid"]')
      .first();
    const isVisible = await catalogCards
      .isVisible({ timeout: 2000 })
      .catch(() => false);

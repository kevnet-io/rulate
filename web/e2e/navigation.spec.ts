...
test.describe("Navigation Workflow", () => {
  ...
  test("should navigate to interactive pages", async ({ page }) => {
    ...
    const exploreExists = await exploreLink
      .isVisible({ timeout: 2000 })
      .catch(() => false);
    ...
    const matrixLink = page
      .locator('a:has-text("Matrix"), a:has-text("matrix")')
      .first();
    const matrixExists = await matrixLink
      .isVisible({ timeout: 2000 })
      .catch(() => false);
  ...
  test("should handle browser back button", async ({ page }) => {
    ...
    const exists = await schemasLink
      .isVisible({ timeout: 2000 })
      .catch(() => false);
  ...
  test("should display active navigation indicator", async ({ page }) => {
    ...
    const activeNav = page
      .locator('[class*="active"], [class*="current"], [class*="selected"]')
      .first();
    const _hasActive = await activeNav
      .isVisible({ timeout: 2000 })
      .catch(() => false);
